...
load_table as (
    ...
    FROM (
        SELECT a.entry_date
            ...
            , substring(a.new_bin_id, 1, 5)                             AS mod
            , a.bay_type
            , a.bin_type_name
            , a.pick_area_name
            , a.bin_height_category
            , datediff(seconds, a.previous_stow_time_utc, a.entry_date) AS cycle_time_secs
            , a.quantity                                                AS units
        from (
                select
                    bee.warehouse_id
